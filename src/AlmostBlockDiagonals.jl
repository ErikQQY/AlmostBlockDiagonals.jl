...
"""
@concrete struct AlmostBlockDiagonal{T, I <: Integer, V <: AbstractMatrix{T}} <:
                 AbstractMatrix{T}
    blocks::Vector{V}
    lasts::Vector{I}
    rows::Vector{I}
    ...
    function AlmostBlockDiagonal{T, I, V}(blocks::Vector{V}, lasts, rows,
            cols) where {T, I <: Integer, V <: AbstractMatrix{T}}
        return new{T, I, V}(blocks, lasts, rows, cols)
    end
end

function AlmostBlockDiagonal(
        blocks::Vector{V}, lasts::Vector{I}) where {T, I <: Integer, V <: AbstractMatrix{T}}
...
"""
@concrete struct IntermediateAlmostBlockDiagonal{T, I <: Integer, V <: AbstractMatrix{T}} <:
                 AbstractMatrix{T}
    blocks::Vector{V}
    lasts::Vector{I}
    rows::Vector{I}
    ...
    function IntermediateAlmostBlockDiagonal{T, I, V}(blocks::Vector{V}, lasts, rows, cols,
            fillers) where {T, I <: Integer, V <: AbstractMatrix{T}}
        return new{T, I, V}(blocks, lasts, rows, cols, fillers)
    end
end

function IntermediateAlmostBlockDiagonal(blocks::Vector{V}, lasts::Vector{I},
        fillers::Vector{I}) where {T, I <: Integer, V <: AbstractMatrix{T}}
    rows_and_cols = size.(blocks)
    rows = first.(rows_and_cols)
    cols = last.(rows_and_cols)
...
function IntermediateAlmostBlockDiagonal(
        blocks::Vector{V}, lasts::Vector{I}) where {T, I <: Integer, V <: AbstractMatrix{T}}
    rows_and_cols = size.(blocks)
    rows = first.(rows_and_cols)
    cols = last.(rows_and_cols)
...
function blocksize(A::AlmostBlockDiagonal, p::Integer, q::Integer)
    size(blocks(A)[p], 1), size(blocks(A)[q], 2)
end
...
function Base.size(A::AlmostBlockDiagonal)
    (sum(size(A.blocks[i], 1) for i in 1:nblocks(A)), sum(A.lasts))
end
...
function Base.getindex(A::AlmostBlockDiagonal, i::Integer, j::Integer)
    ...
    jᵦ = (nth_block_row == 1) ? A.cols[1] :
         accumulate_lasts[nth_block_row - 1] + A.cols[nth_block_row]
    if jₐ < j <= jᵦ
...
function Base.:*(A::AlmostBlockDiagonal{T, I, V},
        x::AbstractVector{S}) where {I <: Integer, V <: AbstractArray, T, S}
    MA = Matrix(A) * x
    return MA
end
...
function Base.zero(A::AlmostBlockDiagonal{
        T, I, V}) where {V <: AbstractArray, I <: Integer, T}
...
function Base.zero(A::IntermediateAlmostBlockDiagonal{
        T, I, V}) where {V <: AbstractArray, I <: Integer, T}
...
function getblock(A::AlmostBlockDiagonal{T}, p::Integer, q::Integer) where {T <: Number}
...
function Base.size(A::IntermediateAlmostBlockDiagonal)
    (sum(size(A.blocks[i], 1) for i in 1:nblocks(A)), sum(A.lasts))
end
...
function Base.setindex!(A::AlmostBlockDiagonal, v, i, j)
    ...
    if jₐ < j <= jᵦ
        ...
        throw(ArgumentError("Cannot set entry ($i, $j) in off-almost-diagonal-block to nonzero value $v."))
    end
end
...
function Base.:/(A::AlmostBlockDiagonal, n::Number)
    AlmostBlockDiagonal(map(x -> x / n, blocks(A)), A.lasts)
end
...
getblock(IA::IntermediateAlmostBlockDiagonal, p::Integer) = blocks(IA)[p]
function getblock(
        IA::IntermediateAlmostBlockDiagonal{T}, p::Integer, q::Integer) where {T <: Number}
    return p == q ? blocks(IA)[p] : zeros(T, blocksize(IA, p, q))
end
...
"""
function factor_shift(IA::IntermediateAlmostBlockDiagonal{T},
        ipivot::AbstractArray{I}, scrtch) where {I <: Integer, T}
    info = 0
    indexx = 1
    i = 1
    nbloks = nblocks(IA)
...
function factor(w::AbstractArray{T}, ipivot::AbstractArray{I},
        d, last::I, info::I) where {I <: Integer, T}
    nrow, ncol = size(w)
    d[1:nrow] .= 0.0 # don't reassign values
...
"""
function substitution(IA::IntermediateAlmostBlockDiagonal, ipivot::AbstractArray{I},
        x::AbstractArray{T}) where {I <: Integer, T <: Real}
    ...
    for i in 1:nbloks
        ...
        @views bloks = IA[i]
        @views forward_substitution(
            bloks, ipivot[indexx:(indexx + last - 1)], last, x[indexx:(indexx + nrow - 1)])
...
function substitution(IA::IntermediateAlmostBlockDiagonal, ipivot::AbstractArray{I},
        vx::Vector{Vector{T}}) where {I <: Integer, T}
    x = reduce(vcat, vx)
    #  forward substitution
...
function forward_substitution(
        w::AbstractArray{T}, ipivot::AbstractArray{I}, last::I, x) where {I <: Integer, T}
    nrow, _ = size(w)
    nrow == 1 && return
